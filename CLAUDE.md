...
- `node e2e/smoke.mjs <projectPath> <shotsDir>` — launches the built app and screenshots every screen (set `VEXA_USER_DATA` to a temp dir)
- `node e2e/features.mjs <projectPath> <shotsDir>` — checks the onboarding tour, branch create/delete and view scrolling (fresh `VEXA_USER_DATA`)
...
- The version and CHANGELOG are written by Vexa on Accept (not by agents) so parallel branches never conflict on them.
